...
async def upload_audio(
    ...
    try:
        ...
        analysis_result = _audio_analyzer.analyze(audio_file)
        logger.info(f"[{session_id}] 音声解析完了: F0={analysis_result.f0_mean_hz:.1f}Hz")

        # --- Step 2: 診断 ---
        diagnosis_result = _diagnosis_engine.diagnose(analysis_result)
        logger.info(f"[{session_id}] 診断完了: type={diagnosis_result.archetype_name}")

        # --- Step 3: PDF生成 ---
        pdf_report = _report_generator.generate(diagnosis_result, user_name)
